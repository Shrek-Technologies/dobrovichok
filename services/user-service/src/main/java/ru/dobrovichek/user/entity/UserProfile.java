...
public class UserProfile {
    ...
    /**
     * Volunteer abandoned an accepted request: current {@link #rating} is multiplied by {@code multiplier}
     * (e.g. 0.8). {@link #ratingCount} is unchanged — this is not a new review.
     */
    public void applyAbandonmentRatingMultiplier(BigDecimal multiplier, Instant now) {
        if (role != UserRole.VOLUNTEER) {
            throw new IllegalStateException("Only volunteer profile can have rating");
        }
        BigDecimal m = Objects.requireNonNull(multiplier);
        if (m.signum() < 0) {
            throw new IllegalArgumentException("multiplier must be non-negative");
        }
        BigDecimal next = rating.multiply(m).setScale(2, RoundingMode.HALF_UP);
        if (next.signum() < 0) {
